...
string algorithm_code = "2V-CHILP-DTD";
float avg2eccSize = 0;
...
inline void insert_element_to_stack(int x,int y){
//     printf("add element %d to stack %d in pos %d / %d\n",y,x, L_current_pos, n);
	int next_empty_slot = L_Next[L_current_pos];
    if(L_First[x]==0){
        L_Target[L_current_pos] = y;
        L_Next[L_current_pos] = 0;
        L_Before[L_current_pos] = 0;
        L_First[x] = L_current_pos;
    }
    else{
        L_Target[L_current_pos]=y;
        L_Next[L_current_pos]=L_First[x];
	    L_Before[L_First[x]]=L_current_pos;
        L_First[x]=L_current_pos;
    }
    L_Pos[y] = L_current_pos;
    L_Stack[y] = x;
    L_current_pos = next_empty_slot;
}

inline void pop_element_from_stack(int x,int y, int pos){
//     printf("pop element %d from stack %d\n",y,x);
    if(L_First[x]==pos){
        L_First[x]=L_Next[pos];
	L_Before[L_Next[pos]] = 0;
    }
    else if(L_Next[pos]==0) {
        L_Next[L_Before[pos]] = L_Next[pos];
    }
    else{
        L_Next[L_Before[pos]] = L_Next[pos];
		L_Before[L_Next[pos]] = L_Before[pos];
    }
    L_Next[pos] = L_current_pos;
    L_current_pos = pos;
...
int slt(int r1, int num_of_vertices, int *idom, bool *isSap) {
...
// 	stack_Cond[x] = true;
	...
	stack_Cond_2[x] = true;
// 	stack_Cond[r1] = true;
	stack_Cond_0[r1] = true;
	stack_Cond_1[r1] = true;
	stack_Cond_2[r1] = true;
...
int find_SAPS(int vertex, int size_of_SCC){
    int * idom = parent;
    int num_SAP_Forward = slt(vertex, size_of_SCC, idom, SAP_in_Forward);
    ...
    int num_SAP_Reverse = slt(vertex, size_of_SCC, r_idom, SAP_in_Reverse);
    ...
    for(int i=0; i <temp_stack_top; i++){
		...
		int v = Target[j];
		int future_next  = Next[j];
		if((r_idom[v] != u && r_idom[u] != v && r_idom[u] != r_idom[v]) || (idom[v] != u && idom[u] != v && idom[u] != idom[v])){
		    //split
		    delete_arc_from_graph(u,v,j);
		    stack_Cond_0[v] = true;
		    stack_Cond_1[v] = true;
		    stack_Cond_2[v] = true;
		}
	...
	if(num_SAP_Forward > num_SAP_Reverse){
	  for(int i=0; i <temp_stack_top; i++){
	    ...
	    if(SAP_in_Forward[SAP]){
		  ...
		  stack_Cond_1[x] = true;
		  stack_Cond_2[x] = true;
		  stack_Cond_0[SAP] = true;
		  stack_Cond_1[SAP] = true;
		  stack_Cond_2[SAP] = true;
...
void BFS (int u, int *Target, int *Next, int *First, bool *marked_S2_Cond){
    ...
    while(q_out!=q_in) {
// 	printf("checking vertex %d (F_two = %d / %d)\n",queue_BFS[q_out],F_two, delta);
...
int OneVertexOut(int u, int threshold, int *Target, int *Next, int *First, bool *marked_S1_Cond, bool *marked_S2_Cond){
    ...
    delta = pow(2,threshold);
    check=2*delta+1;
    
//     for(int i = 0; i < temp_stack_top; i++){
// 	int q = temp_stack[i];
// 	if(marked_BFS[q]!= 0){
// 	  printf("marked_BFS[%d] is not initialized\n",q);
// 	  exit(1);
// 	}
// 	if(w[q]!= 0){
// 	  printf("w[%d] is not initialized\n",q);
// 	  exit(1);
// 	}
//     }
...
//  	  printf("The second search visited %d edges, delta = %d\n",F_two,delta);
	  for(int i = 1; i < p_S2; i++){
		  int q = marked_S2[i];
		  int k=First[q];
		  while(k!=0){
		      int v=Target[k];
		      int future_next = Next[k];
		      if(!marked_S2_Cond[v] && q != x_sp){ printf("1) edges (%d,%d) ==> (%d,%d) is exiting the component where x = %d\n",q,v,real_arc[q],real_arc[v],real_arc[x_sp]); exit(1);}
// 		      if(!marked_S2_Cond[v] && q == x_sp) printf("2) edges (%d,%d) is exiting the component where x = %d\n",real_arc[q],real_arc[v],real_arc[x_sp]);
		      k=future_next;
		  }
//  		  marked_BFS[q] = 0;
// 		  w[q] = 0;
...
// 	   for (int i=0;i<k;k++) {
// 	      if (x_matrix[parent[w_matrix[k]]] == true && x_matrix[w_matrix[k]]== false) {
// 		  x_sp = w_matrix[k]; 
// 		  break;
// 	      }
// 	    }
...
void Process_OneVertexOut(int vertex, int size_of_SCC, int num_of_edges, int recursionDepth, int qualityOfRecursion) {

  if (size_of_SCC == 1) return;
  if(recursionDepth > maxRecursionDepth) maxRecursionDepth = recursionDepth;
  numOfRecursionCalls ++;
    
//    printf("Process_OneEdgeOut(%d,%d,%d, %d, %d)\n",vertex,size_of_SCC,num_of_edges,recursionDepth, qualityOfRecursion);
   int component_type;
...
//          for(int i = 0; i < temp_stack_top; i++) temp_stack[i] = real_arc[ temp_stack[i]];
//          SortNumbers(temp_stack, temp_stack_top, n);
//          printf("\n2vcc (%d vertices) : ",temp_stack_top);
...
//          printf("\n");
  ...
  int QualityBound;
  if(qualityOfRecursion == 2) 		QualityBound = ceil(pow(num_of_edges,0.25));
  else if(qualityOfRecursion == 1) 	QualityBound = ceil(pow(num_of_edges,0.33));
  else 					QualityBound = ceil(pow(num_of_edges,0.5));
  
  if(QualityBound < 4) QualityBound = 4;
    
//   printf("QualityBound= %d\n",QualityBound);
  int stackBound = (ceil(log2(QualityBound)));
...
//   int Bound = (ceil(log2(sqrt(num_of_edges))));
    for(int i = 0; i < temp_stack_top; i++){
	    if(qualityOfRecursion == 2 && stack_Cond_2[temp_stack[i]]){
	      insert_element_to_stack(1,temp_stack[i]);
	      stack_Cond_2[temp_stack[i]]=false;
	    }
	    else if(qualityOfRecursion == 1 && stack_Cond_1[temp_stack[i]]){
	      insert_element_to_stack((ceil(log2(pow(num_of_edges,0.25)))),temp_stack[i]);
	      stack_Cond_1[temp_stack[i]]=false;
	      stack_Cond_2[temp_stack[i]]=false;
	    }
	    else if(qualityOfRecursion == 0 && stack_Cond_0[temp_stack[i]]){
	      insert_element_to_stack((ceil(log2(pow(num_of_edges,0.33)))),temp_stack[i]);
	      stack_Cond_0[temp_stack[i]]=false;
	      stack_Cond_1[temp_stack[i]]=false;
	      stack_Cond_2[temp_stack[i]]=false;
	    } 
	    
	    in_small_comp[temp_stack[i]] = false;
    }
   ...
   Nodes_before = Nodes + j_global;
    ...
    while (1){
	    int u = 0;
	    int prev_stack_of_u = 0;
	    
	    for(int i = 1; i <= stackBound; i++){
		    if(L_First[i] == 0) continue;
		    else{
			    u = L_Target[L_First[i]];
			    prev_stack_of_u = i;
			    pop_element_from_stack(i,u,L_First[i]);
			    break;
		    }
	    }
	    ...
	    if(u == 0) break;
	      ...
	      if(qualityOfRecursion == 2)	stack_Cond_2[u]=false;
	      else if(qualityOfRecursion == 1){
		stack_Cond_1[u] = false;
		stack_Cond_2[u] = false;
	      }
	      else {
		stack_Cond_0[u]=false;
		stack_Cond_1[u] = false;
		stack_Cond_2[u] = false;
	      }
// 	      if(qualityOfRecursion == 0)stack_Cond[u]=false;
// 	      printf("\n check for component vertex %d round %d / %d\n",u,prev_stack_of_u,Bound);
	      number_of_nodes_of_1egeOut=0;
	      p_S1 = p_S2 = 0;
	      
	      component_type = OneVertexOut(u, prev_stack_of_u-1, Target, Next, First, marked_S1_Cond, marked_S2_Cond);
		...
		if (component_type != 0){
		      ...
		      if (idf==2){
			    ...
			    marked_BFS[x] = 0;
			    marked_Cond[x] = false;
			    marked_S1_Cond[temp_stack_top-1] = false;
			    marked_S2_Cond[temp_stack_top-1] = false;
			    
			    if(!in_small_comp[x_sp]){
				if(L_Stack[x_sp] > 1){
				    //add their endpoints in stack
				    pop_element_from_stack(L_Stack[x_sp],x_sp,L_Pos[x_sp]);
				    insert_element_to_stack(1,x_sp);
				    
				}
				else if(L_Stack[x_sp] == 0){
				    insert_element_to_stack(1,x_sp);
				}
				if(L_Stack[x_sp] != 0){
				    stack_Cond_0[x_sp] = true;
				    stack_Cond_1[x_sp] = true;
				    stack_Cond_2[x_sp] = true;
				}
			    }
		      }
			    ...
			    while(l!=0){
				  ...
				  if (idf==1){
				    if(marked_S1_Cond[v]==false){
				      ...
				      if(!in_small_comp[v]){
// 					  if(qualityOfRecursion != 0) stack_Cond[v]=true;
					  if(L_Stack[v] > 1){
					      //add their endpoints in stack
					      pop_element_from_stack(L_Stack[v],v,L_Pos[v]);
					      insert_element_to_stack(1,v);
					  }
					  else if(L_Stack[v] == 0){
					      insert_element_to_stack(1,v);
					  }
...
// 			    stack_Cond[S_one[i]] = false;
			    stack_Cond_0[S_one[i]] = false;
			    stack_Cond_1[S_one[i]] = false;
			    stack_Cond_2[S_one[i]] = false;
			    if(L_Stack[S_one[i]]!=0){
				    pop_element_from_stack(L_Stack[S_one[i]],S_one[i],L_Pos[S_one[i]]);
			    }
		    }
		...
		if(component_type == 0){
		    ...
		    p_S1 = p_S2 = 0;
		    component_type_r = OneVertexOut(u, prev_stack_of_u-1, r_Target, r_Next, r_First, marked_S1_Cond, marked_S2_Cond);
...
// 				  printf("Added vertex (%d) %d/%d\n",x,temp_stack_top,n);
				  ...
				  if(!in_small_comp[x_sp]){
// 					if(qualityOfRecursion != 0) stack_Cond[x_sp]=true;
					if(L_Stack[x_sp] > 1){
					    //add their endpoints in stack
					    pop_element_from_stack(L_Stack[x_sp],x_sp,L_Pos[x_sp]);
					    insert_element_to_stack(1,x_sp);
...
// 				    stack_Cond[S_one_r[i]] = false;
				    stack_Cond_0[S_one_r[i]] = false;
				    stack_Cond_1[S_one_r[i]] = false;
				    stack_Cond_2[S_one_r[i]] = false;
				    if(L_Stack[S_one_r[i]]!=0){
					    pop_element_from_stack(L_Stack[S_one_r[i]],S_one_r[i],L_Pos[S_one_r[i]]);
				    }
			      }
			}
			else{
			    if(prev_stack_of_u < stackBound){
				    insert_element_to_stack(prev_stack_of_u+1, u);
			    } 
			}	
...
// 	printf("SCC %d / %d with %d vertices, one vertex in the SCC is %d\n",i,currentSccID-1,num_in_SCC[i],vertex_in_SCC[i]);
    ...
    for(int i = 1; i < currentSccID; i++){
	if(edges_in_SCC[i] < ceil(pow(num_of_edges,0.5))){
	    for(int j = SCCs_first[i]; j < SCCs_first[i+1]; j++){ // for every vertex in the SCC
		x = SCCs[j];
		in_small_comp[x] = true;
		stack_Cond_0[x] = false;
		stack_Cond_1[x] = false;
		stack_Cond_2[x] = false;
	    }
	}
    }
...
// 	    Process_OneVertexOut(vertex_in_SCC[i], num_in_SCC[i], edges_in_SCC[i],recursionDepth+1,);
// 	    printf("Call %d-th SCC (out of %d), n = %d, m = %d, m now is %d, RecursionBound = %d\n",i,currentSccID-1, num_in_SCC[i], edges_in_SCC[i], num_of_edges, QualityBound);
	    if(edges_in_SCC[i] <= (num_of_edges - ceil(pow(num_of_edges,0.5))))	Process_OneVertexOut(vertex_in_SCC[i], num_in_SCC[i], edges_in_SCC[i],recursionDepth+1, 2);
	    else{
		    if(qualityOfRecursion > 0) Process_OneVertexOut(vertex_in_SCC[i], num_in_SCC[i], edges_in_SCC[i],recursionDepth, qualityOfRecursion - 1);
		    else Process_OneVertexOut(vertex_in_SCC[i], num_in_SCC[i], edges_in_SCC[i],recursionDepth+1, qualityOfRecursion);
	    }
	}
    }
...
void init() {
    ...
    L_First = new int [n+2];
    L_Target = new int [n+2];
    L_Next = new int [n+2];
    L_Before = new int [n+2];
    L_Stack = new int [n+2];
    L_Pos = new int [n+2];
    ...
    stack_Cond_0=new bool [n+2];
    stack_Cond_1=new bool [n+2];
    stack_Cond_2=new bool [n+2];
...
//     double t;
  ...
  //  output = fopen("exit.txt", "w");
    ...
    //slt(1,N);
    k_split = 0;
    Process_OneVertexOut(rand_node,Nodes,M,1,2);
    delete_space();
