...
string algorithm_code = "2V-CHILP-STAR";
float avg2eccSize = 0;
...
void Process_OneVertexOut(int vertex, int size_of_SCC, int num_of_edges, int recursionDepth) {
...
//          printf("\n");
  ...
  int Bound = (ceil(log2(sqrt(sqrt(num_of_edges)))));
  for(int i = 0; i < temp_stack_top; i++){
...
//     double t;
   ...
   // output = fopen("exit.txt", "w");
    ...
    //slt(1,N);
    k_split = 0;
    Process_OneVertexOut(rand_node,Nodes,M,1);
    delete_space();
