...
string algorithm_code = "2E-CHILPSTAR";
float avg2eccSize = 0;
...
inline void insert_element_to_stack(int x,int y){
    //printf("add element %d to stack %d\n",y,x);
...
// 
//         if(temp_stack_top > max2vcbSize) max2vcbSize = temp_stack_top;
...
int OneEdgeOut(int u, int threshold, int *Target, int *Next, int *First, bool *marked_S1_Cond, bool *marked_S2_Cond){
    next=0;
    F_one=0;
    marked_S1[1]=u;
    ...
    if (F_one < check){
      ...
      return idf;
		...
		if (F_two  <= delta){
			...
			idf=2;
			for(int i=next;i>1;i--){
				if(w[pre2label[i]]>=delta && !marked_S2_Cond[pre2label[i]] && marked_S2_Cond[parent[pre2label[i]]]){
...
void Process_OneEdgeOut(int vertex, int size_of_SCC, int num_of_edges, int recursionDepth){
	...
	while (1){
		...
		if(u == 0){printf("This should not happen!!"); exit(1);}
		if(u == -1) break;
		...
		component_type = OneEdgeOut(u, prev_stack_of_u-1, Target, Next, First, marked_S1_Cond, marked_S2_Cond);
		
		//printf("number_of_nodes_of_1egeOut= %d, check=%d\n ",number_of_nodes_of_1egeOut,check);
		
		if (component_type != 0){
			number_of_components = number_of_components+1;
			CompFound[prev_stack_of_u-1]++;
			...
			if(idf == 2){
				...
				while(l!=0){
					if(Target[l]==outgoing_edge_2){
					  ...
					  if(!in_small_comp[outgoing_edge_2]){
					      if(L_Stack[outgoing_edge_2] > 1){
						  //add their endpoints in stack
						  pop_element_from_stack(L_Stack[outgoing_edge_2],outgoing_edge_2,L_Pos[outgoing_edge_2]);
						  RemovedFromQueue[L_Stack[outgoing_edge_2]]++;
						  insert_element_to_stack(1,outgoing_edge_2);
					      }
					      else if(L_Stack[outgoing_edge_2] == 0){
						  insert_element_to_stack(1,outgoing_edge_2);
					      }
					  }
					}
			...
			for (int i=1;i<number_of_nodes_of_1egeOut;i++){
				...
				if(L_Stack[S_one[i]]!=0){
					...
					RemovedFromQueue[L_Stack[S_one[i]]]++;
				}
			}
		}
		...
		if(component_type == 0){
			...
			p_S1 = p_S2 = 0;
			component_type_r = OneEdgeOut(u, prev_stack_of_u-1 , r_Target, r_Next, r_First, marked_S1_Cond, marked_S2_Cond);
			...
			if (component_type_r != 0){
				...
				for (int i=1;i<number_of_nodes_of_1egeOut;i++){
					...
					if(L_Stack[S_one_r[i]]!=0){
						...
						RemovedFromQueue[L_Stack[S_one_r[i]]]++;
					}
				}
			}
			else{
				ThresholdIncreased[prev_stack_of_u-1]++;
				if(prev_stack_of_u < Bound){
...
void init() {
	...
	// 	r_pre2label[i]=0;
		...
		stack_Cond[i] = false; // here it might as well be set to true, which would mean that in the beginning of the algorithm there will be searches from each vertex
	...
	CompFound = new int[(int)ceil(log2(sqrt(m)))+2];
	ThresholdIncreased = new int [(int)ceil(log2(sqrt(m)))+2];
	RemovedFromQueue = new int [(int)ceil(log2(sqrt(m)))+2];
	
    for (int i = 0; i < ceil(log2(sqrt(m)))+2; i++){
    	CompFound[i] = 0;
    	ThresholdIncreased[i] = 0;
    	RemovedFromQueue[i] = 0;
    }
}
...
void readGraph(const char *file) {
 ...
 int main(int argc, char *argv[]) {
    ...
    for (int i = 0; i <= ceil(log2(sqrt(m))); i++){
    	printf("\n+ %d 1EdgeOut Comps of size %f were found\n",CompFound[i],pow(2,i));
    	printf("- %d searches for 1EdgeOut Comps of size %f failed\n",ThresholdIncreased[i],pow(2,i));
    	printf("0 %d vertices were stopped before searching for a comp of size %f\n",RemovedFromQueue[i],pow(2,i));
    }
